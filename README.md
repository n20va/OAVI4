# Лабораторная работа №4  
## Выделение контуров на изображении

---

## Вариант 11  
Метод: оператор Круна (Kroon) 3×3  
Формула градиента:  
\[
G = \sqrt{G_x^2 + G_y^2}
\]

---

## Цель работы
Изучить методы выделения контуров на изображении и реализовать градиентный алгоритм с использованием оператора Круна.

---

## Исходные данные

- Источник изображения:  
`https://www.slavcorpora.ru`
- Тип изображения: цветное (RGB)
- Размер: определяется автоматически

---

## Этапы выполнения

### 1. Преобразование изображения в полутоновое

Используется формула:

\[
I(x, y) = 0.299R + 0.587G + 0.114B
\]

---

### 2. Вычисление градиентов

Используются ядра оператора Круна:

#### Горизонтальный градиент \(G_x\)

\[
G_x =
\begin{bmatrix}
17 & 61 & 17 \\
0 & 0 & 0 \\
-17 & -61 & -17
\end{bmatrix}
\]

#### Вертикальный градиент \(G_y\)

\[
G_y =
\begin{bmatrix}
17 & 0 & -17 \\
61 & 0 & -61 \\
17 & 0 & -17
\end{bmatrix}
\]

---

### 3. Вычисление итогового градиента

\[
G = \sqrt{G_x^2 + G_y^2}
\]

---

### 4. Нормализация

Все матрицы \(G_x\), \(G_y\), \(G\) приводятся к диапазону [0, 255].

---

### 5. Бинаризация

\[
B(x,y) =
\begin{cases}
255, & G(x,y) \ge T \\
0, & иначе
\end{cases}
\]

где \(T = 110\)

---

## Результаты

### Исходное изображение
![source](src/source_color.png)

---

### Полутоновое изображение
![gray](src/grayscale.bmp)

---

### Градиенты

| Gx | Gy | G |
|:--:|:--:|:--:|
| ![](src/gx_norm.bmp) | ![](src/gy_norm.bmp) | ![](src/g_norm.bmp) |

---

### Бинаризация
![binary](src/g_binary.bmp)

---

## Вывод

В ходе лабораторной работы был реализован алгоритм выделения контуров с использованием оператора Круна.

Были получены частные производные изображения \(G_x\) и \(G_y\), итоговая градиентная матрица \(G\), а также бинаризованная карта контуров.

Метод показал хорошее качество выделения границ объектов и устойчивость к шуму по сравнению с базовыми операторами.
